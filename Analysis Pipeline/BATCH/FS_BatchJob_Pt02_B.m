...
disp('Performing Motion Correction transform Calculation on day 1');
  X2 = MaxProj(:,:,1);
...
IO = 1;
  for iii=1:length(mov_listing)
        ...
        for ii = 1:size(mov_data,2) % format movie data
          ...
          mov_data_aligned(ii).cdata(:,:,:) = mov_data2(:,:,ii);   % store rgb correceted data here temporarily...
        end

       X4(:,:,iii) = max(mov_data2,[],3); %v Store all the Max proj images
...
X5 = mean(X4,3);
tform = imregtform(X5,X3,'rigid',optimizer,metric); %Apply Max projection comparison across days

%%--- Loop through each movie and perform intensity based image correction, based on aligning the average max projection.
   for  iv = 1:size(X4,3)

     [path,file,ext]=fileparts(filenames{iv});
     load(fullfile(pwd,mov_listing{iv}),'mov_data_aligned');  %load movie data

     MAX_dat = X4(:,:,iv); % Get current MAX projection

        for ii = 1:size(mov_data,2)
          mov_data_aligned_actual(ii).cdata(:,:,:) = imwarp(mov_data_aligned(:,:,ii),tform,'OutputView',imref2d(size(X5))); % Align Video
        end

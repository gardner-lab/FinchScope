function FS_BatchJob_TemplateMatch(TEMPLATE,varargin)
%FS_BatchJob_TemplateMatch allows the user to select a song template, and then attempts to find
%matches to the template through a running Euclidean distance score (similar to SAP)

% This is the Batch-Job (paired down) version of this script Works with functions to
%		-- text the user durring when user input is required.
%		-- text the user if an error occurs in the pipeline, or when the batch jobs are complete
%
%	based on the function by Jeff Markowitz: fb_template_match(TEMPLATE,varargin)
%
%	TEMPLATE

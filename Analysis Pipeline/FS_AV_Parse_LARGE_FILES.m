...
vid.Duration
% Make chunks
chunk_size = round(vid.Duration/20+1);
...
fs = 48000;
		...
		imwrite(flipdim(uint8(s(minpt:maxpt,:)),1),hot,fullfile(gif_dir,[fileN '.gif']),'gif');
		save(fullfile(mat_dir,[fileN '.mat']),'audio','video','-v7.3');
...
disp(strcat('Processing chunk: ',num2str(n),'of ',num2str(chunk_size)));
end
